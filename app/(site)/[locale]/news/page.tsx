...
export default async function IndexPage({ params }: { params: Params }) {
  ...
  return (
    <section className="space-y-4">
      <PageHeader title={locale==="ja" ? "ニュース" : "News"} hint={locale==="ja" ? "最新順 50件まで" : "Up to 50 recent items"} />
      {list.length === 0 ? (
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {list.map((it) => (
            <ItemCard
              ...
              date={it._createdAt}
              excerpt={it.excerpt}
              href={it.slug ? `/${locale}/news/${encodeURIComponent(it.slug)}` : undefined}
            />
          ))}
        </div>
      )}
