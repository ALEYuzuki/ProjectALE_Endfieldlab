    ...
    <div className="mx-auto max-w-[1440px] px-4 py-6 text-slate-100 md:px-8">
      <div className="border border-slate-700 bg-slate-950/95 shadow-[0_0_0_1px_rgba(148,163,184,0.08)_inset]">
        ...
        <main className="grid gap-4 p-4 md:grid-cols-12 md:p-6">
          ...
          <Panel className="md:col-span-4" title="Enemy Status" subtitle="Defense model and enemy coefficients">
            <div className="grid gap-3">
              <SelectField
          ...
          <Panel className="md:col-span-12" title="Buff Status" subtitle="Bucket table editor">
            <div className="space-y-2">
              <div className="grid grid-cols-12 gap-2 px-1 text-[10px] uppercase tracking-[0.14em] text-slate-500">
                <span className="col-span-1">ON</span><span className="col-span-3">Name</span><span className="col-span-4">Bucket</span><span className="col-span-3">Value</span><span className="col-span-1">Del</span>
              </div>
              {buckets.map((bucket, idx) => (
                <div key={bucket.id} className="grid grid-cols-12 gap-2 border border-slate-700 bg-[#0a141f] p-2">
                  <label className="col-span-1 flex items-center justify-center border border-slate-600 bg-[#07111a]"><input type="checkbox" checked={bucket.enabled} onChange={(e) => updateBucket(idx, { enabled: e.target.checked })} /></label>
                  <input className="col-span-3 border border-slate-600 bg-[#07111a] px-2 py-1 text-xs text-slate-100 placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none" value={bucket.label} onChange={(e) => updateBucket(idx, { label: e.target.value })} placeholder="buff name" />
                  <select className="col-span-4 border border-slate-600 bg-[#07111a] px-2 py-1 text-xs text-slate-100 focus:border-cyan-400 focus:outline-none" value={bucket.bucket} onChange={(e) => updateBucket(idx, { bucket: e.target.value as BuffBucketKey })}>
                    <option className="bg-slate-900 text-slate-100" value="atk_percent">atk_percent</option><option className="bg-slate-900 text-slate-100" value="damage_bonus">damage_bonus</option><option className="bg-slate-900 text-slate-100" value="elemental_damage_bonus">elemental_damage_bonus</option><option className="bg-slate-900 text-slate-100" value="skill_damage_bonus">skill_damage_bonus</option><option className="bg-slate-900 text-slate-100" value="enemy_damage_taken">enemy_damage_taken</option><option className="bg-slate-900 text-slate-100" value="final_multiplier">final_multiplier</option>
                  </select>
                  <input className="col-span-3 border border-slate-600 bg-[#07111a] px-2 py-1 text-xs text-slate-100 placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none" type="number" step="0.01" value={bucket.value} onChange={(e) => updateBucket(idx, { value: Number(e.target.value) })} />
                  <button className="col-span-1 border border-rose-700/70 bg-rose-950/30 text-[11px] text-rose-300 hover:border-rose-400" onClick={() => setBuckets((prev) => prev.filter((_, i) => i !== idx))}>×</button>
                </div>
              ))}
              <div className="pt-1"><button className="border border-slate-600 bg-slate-900 px-3 py-1 text-xs text-slate-100 hover:border-cyan-400" onClick={() => setBuckets((prev) => [...prev, { id: crypto.randomUUID(), label: "New Bucket", bucket: "damage_bonus", value: 0.1, enabled: true }])}>{isJa ? "Add Buff / バフ追加" : "Add Buff"}</button></div>
            </div>
          </Panel>
        </main>
        ...
        <section className="border-t border-slate-700 px-4 py-4 md:px-6 md:py-5">
          ...
          <div className="mb-4 grid gap-2 md:grid-cols-5">
            <MetricCard label="FINAL TOTAL DAMAGE" value={result.finalTotalDamage.toLocaleString()} emphasis />
            <MetricCard label="PER HIT DAMAGE" value={result.roundedPerHitDamage.toFixed(2)} />
            <MetricCard label="FINAL ATK" value={result.finalAtk.toFixed(2)} />
            <MetricCard label="ENEMY COEFFICIENT" value={result.enemyCoefficientAfterAdjustment.toFixed(4)} />
            <MetricCard label="SELF BUFF" value={result.selfBuffMultiplier.toFixed(4)} />
          </div>
...
function NumberField({ label, value, setValue, step = 1 }: { label: string; value: number; setValue: (v: number) => void; step?: number }) {
  return (
    <label className="text-xs">
      ...
      <input className="w-full border border-slate-600 bg-[#07111a] px-2 py-1.5 text-sm text-slate-100 placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none" type="number" step={step} value={value} onChange={(e) => setValue(Number(e.target.value))} />
    </label>
  );
}
...
function TextNumberField({ label, value, setValue }: { label: string; value: string; setValue: (v: string) => void }) {
  return (
    <label className="text-xs">
      <div className="mb-1 text-slate-400">{label}</div>
      <input className="w-full border border-slate-600 bg-[#07111a] px-2 py-1.5 text-sm text-slate-100 placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none" type="text" value={value} onChange={(e) => setValue(e.target.value)} placeholder="(optional)" />
    </label>
  );
}
...
function SelectField({ label, value, setValue, options }: { label: string; value: string; setValue: (v: string) => void; options: Array<{ value: string; label: string }> }) {
  return (
    <label className="text-xs">
      <div className="mb-1 text-slate-400">{label}</div>
      <select className="w-full border border-slate-600 bg-[#07111a] px-2 py-1.5 text-sm text-slate-100 placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none" value={value} onChange={(e) => setValue(e.target.value)}>
...
function MetricCard({ label, value, emphasis = false }: { label: string; value: string; emphasis?: boolean }) {
  return <div className={`border p-2 ${emphasis ? "border-cyan-500/60 bg-cyan-950/20" : "border-slate-700 bg-slate-900/60"}`}><div className="text-[10px] uppercase tracking-[0.14em] text-slate-400">{label}</div><div className={`mt-1 text-sm font-semibold ${emphasis ? "text-cyan-200" : "text-slate-100"}`}>{value}</div></div>;
}
